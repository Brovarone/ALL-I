                                ...
                                Using dtBancheCli As DataTable = CaricaSchema("MA_Banks", True, True, "Select ABI, CAB, Bank, IsACompanyBank, Description , TBCreatedID, TBModifiedID FROM MA_Banks where IsACompanyBank=0")
                                    ...
                                    Using dsClienti As New DataSet
                                        ...
                                        Using da As New SqlDataAdapter("SELECT Payment, ACGCode, InstallmentType FROM MA_PaymentTerms", Connection)
                                            ...
                                            cmd = New SqlCommand("Select InvoiceAccTpl, AccompanyingInvoiceAccTpl, EUInvoiceAccTpl, CreditNoteAccTpl, EUCreditNoteAccTpl,ExtraEUInvoiceAccTpl, ExtraEUCreditNoteAccTpl  FROM MA_UserDefaultSales WHERE Branch = '*' AND WorkerID = 0", Connection)
                                            da.SelectCommand = cmd
...
#End Region
                                                                            ...
                                                                            Dim isCEE As Boolean = If(iCliFound <> -1, dvClienti(iCliFound)("CustSuppKind"), drCli("CustSuppKind")) = CustSuppKind.CEE
                                                                            ...
                                                                            Select Case .Item("U").ToString.ToUpper
                                                                                ...
                                                                                Case "A"
                                                                                    ...
                                                                                    If isNazionale Then
                                                                                        drDoc("AccTpl") = dtDefVendite.Rows(0).Item("AccompanyingInvoiceAccTpl")
                                                                                    ElseIf isCEE Then
                                                                                        drDoc("AccTpl") = dtDefVendite.Rows(0).Item("EUInvoiceAccTpl")
                                                                                    Else
                                                                                        drDoc("AccTpl") = dtDefVendite.Rows(0).Item("ExtraEUInvoiceAccTpl")
                                                                                    End If
                                                                                    totDoc(0) += 1 ' Fatture
                                                                                Case "F"
                                                                                    drDoc("DocumentType") = DocumentType.Fattura
                                                                                    If isNazionale Then
                                                                                        drDoc("AccTpl") = dtDefVendite.Rows(0).Item("InvoiceAccTpl")
                                                                                    ElseIf isCEE Then
                                                                                        drDoc("AccTpl") = dtDefVendite.Rows(0).Item("EUInvoiceAccTpl")
                                                                                    Else
                                                                                        drDoc("AccTpl") = dtDefVendite.Rows(0).Item("ExtraEUInvoiceAccTpl")
                                                                                ...
                                                                                Case "N"
                                                                                    ...
                                                                                    If isNazionale Then
                                                                                        drDoc("AccTpl") = dtDefVendite.Rows(0).Item("CreditNoteAccTpl")
                                                                                    ElseIf isCEE Then
                                                                                        drDoc("AccTpl") = dtDefVendite.Rows(0).Item("EUCreditNoteAccTpl")
                                                                                    Else
                                                                                        drDoc("AccTpl") = dtDefVendite.Rows(0).Item("ExtraEUCreditNoteAccTpl")
                                                                                    End If
...
#Enable Warning BC42104 ' La variabile è stata usata prima dell'assegnazione di un valore
    ...
    Private Function AggiornaAnagraficaDaClienOrd(origine As DataRow, clienord As DataRowView, anag As DataRowView, o As DataRowView) As MyLogsString
        Dim avvisi As New StringBuilder()
        Dim warnings As New StringBuilder()
        Dim lowWarnings As New StringBuilder() '07/04/2025 normalizzo gli indirizzi quindi li escludo dai log
        Dim mlog As New MyLogsString
