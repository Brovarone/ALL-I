...
Public Module Contabilita
    ...
    ''' <summary>
    ''' Creo la scrittura dei risconti in contabilità. Il file si deve chiamare RISCONTIRIDOTTO
    ''' </summary>
    ...
    End Function
    ''' <summary>
    ''' Creo la scrittura dei risconti in contabilità. Il file si deve chiamare RISCONTI
    ''' </summary>
...
#End Region
                                                                ...
                                                                Try
                                                                    ...
                                                                    If isAnalitico Then
                                                                        ...
                                                                        With drPnD
                                                                            ...
                                                                            Dim iCanoni = If(xlsRidotto, r.Rows(irow).Item("E"), r.Rows(irow).Item("K"))
                                                                            iCanoni = If(iCanoni = 0, 1, Math.Abs(iCanoni))
                                                                            ...
                                                                            Dim CdC As String = If(xlsRidotto, r.Rows(irow).Item("F"), r.Rows(irow).Item("L"))
    ...
    End Function
    ''' <summary>
    ''' Usata solo per flusso derivante da Fusione
    ''' </summary>
    Public Function CreaPNotaRiscontiFusione(ByVal r As DataTable, ByVal withAnalitica As Boolean, Optional ByVal data As Date = Nothing) As Boolean
        'Movimenti Contabili PURI - MA_JournalEntries
        'Righe - MA_JournalEntriesGLDetail
        ...
        Dim DataRiga As Date
        If data = Nothing Then
            DataRiga = MagoFormatta("20230401", GetType(DateTime)).DataTempo
        Else
            DataRiga = data
        End If
        FLogin.prgCopy.Value = 1
...
#End Region
                                                    ...
                                                    Dim drPnD As DataRow = dtPND.NewRow
                                                    Dim isNewReg As Boolean = True
                                                    Dim contoRisconto As String = RiscAtt
                                                    'Ho due gruppi risconti attivi e passivi
                                                    Dim rview As New DataView(r, "J = 'Risconto attivo'", "G", DataViewRowState.CurrentRows)
                                                    For iTipo As Integer = 1 To 2
                                                        If iTipo = 2 Then
                                                            rview = New DataView(r, "J = 'Risconto passivo'", "G", DataViewRowState.CurrentRows)
                                                            contoRisconto = RiscPass
                                                            isNewReg = True
                                                            iLine = 0
                                                        End If
                                                        For irow = i To rview.Count - 1
                                                            With rview(irow)
                                                                iLine += 1
                                                                ...
                                                                drPnD("AccrualDate") = DataRiga
                                                                drPnD("AccRsn") = CauRiga
                                                                'Nota =  centro + numero + data
                                                                Dim nota As String = .Item("I").ToString & " " & .Item("C").ToString & " " & Left(.Item("A"), 10)
                                                                drPnD("Notes") = nota
                                                                Dim isDare As Boolean = iTipo = 1
                                                                Dim sConto As String = .Item("G").ToString
                                                                Dim iCP As Integer = dvCntrp.Find(sConto)
                                                                If iCP = -1 Then
                                                                    Debug.Print("Conto senza corrispondenza: " & sConto)
                                                                    My.Application.Log.WriteEntry("Conto senza corrispondenza:" & sConto)
                                                                ...
                                                                drPnD("DebitCreditSign") = If(isDare, 4980736, 4980737)
                                                                Dim imp As Double = Math.Round(.Item("P"), 2) ' Math.Round(CDbl(.Item("Imponibile")), 2)
                                                                drPnD("Amount") = imp
                                                                drPnD("FiscalAmount") = imp
                                                                quadratura += If(isDare, imp, -imp)
                                                                If isDare Then
                                                                    ...
                                                                    Try
                                                                        ...
                                                                        If isAnalitico Then
                                                                            ...
                                                                            With drPnD
                                                                                ...
                                                                                drAna("TBCreatedID") = My.Settings.mLOGINID 'ID utente
                                                                                drAna("TBModifiedID") = My.Settings.mLOGINID 'ID utente
                                                                                Select Case rview(irow).Item("B")
                                                                                    Case "FR", "NR", "FRREV"
                                                                                        drAna("CustSuppType") = CustSuppType.FornitoreIgnora
                                                                                    Case "FE", "FECEE", "NE"
                                                                                        drAna("CustSuppType") = CustSuppType.ClienteIgnora
                                                                                    Case Else
                                                                                        drAna("CustSuppType") = CustSuppType.IgnoraIgnora
                                                                                End Select
                                                                                drAna("CustSupp") = rview(irow).Item("K")
                                                                                ...
                                                                                Dim tempAmount As Double = 0
                                                                                Dim iLineMovAna As Integer = 0
                                                                                Dim iCanoni = rview(irow).Item("U")
                                                                                iCanoni = If(iCanoni Is Nothing, 1, iCanoni)
                                                                                iCanoni = If(iCanoni = 0, 1, Math.Abs(iCanoni))
                                                                                Dim importoMensile As Double = importo / iCanoni
                                                                                Dim dataDecorrenza As Date = DataRiga
                                                                                Dim isDareAna = drAna("DebitCreditSign") = 8192000
                                                                                Dim CdC As String = rview(irow).Item("V") ' colonna I Cdc Uno senza carattere 1
