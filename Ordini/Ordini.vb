...
#Region "Estrazioni dati con Query LINQ"
            ...
            sw.Start()
            Dim iAllOrders As IQueryable(Of MaSaleOrd)
            If 1 = 2 Then
                'TODO: Eliminare se tutto funziona bene
                Debug.WriteLine("mia")
            ...
            Else
                ...
                ' 1. Definiamo q partendo dalla tabella base (SENZA INCLUDE)
                ' Usiamo lo stesso nome "q" per non dover cambiare tutti i tuoi If/Where successivi
                Dim q = From o In OrdContext.MaSaleOrd

                ' Calcola i limiti fuori dalla query
                Dim dInizio = fromOrdDate.Date
                Dim dFine = toOrdDate.Date.AddDays(1) ' Per includere tutto il giorno "to"
                If bFiltroDateOrdini Then
                    q = q.Where(Function(o) o.OrderDate >= dInizio AndAlso o.OrderDate < dFine)
                Else
                    q = q.Where(Function(o) o.OrderDate < dFine)
                End If
                ...
                ' -------------------------------------------------------------------------
                ' TRUCCO PRESTAZIONALE: Carichiamo i dati in memoria in step separati
                ' -------------------------------------------------------------------------

                ' A. Eseguiamo la query principale (solo MaSaleOrd) per ottenere gli oggetti e i loro ID
                ' NOTA: Non usiamo AsNoTracking qui per permettere il "Relationship Fix-up"
                Dim tempResult = q.ToList()
                Dim ordIds = tempResult.Select(Function(o) o.SaleOrdId).ToList()

                If ordIds.Any() Then
                    ' B. Carichiamo i dettagli (EF li aggancia automaticamente a tempResult)
                    OrdContext.MaSaleOrdDetails.Where(Function(d) ordIds.Contains(d.SaleOrdId)).ToList()

                    ' C. Carichiamo le tabelle custom (EF le aggancia automaticamente)
                    OrdContext.AllordCliAcc.Where(Function(a) ordIds.Contains(a.IdOrdCli)).ToList()
                    OrdContext.AllordCliDescrizioni.Where(Function(d) ordIds.Contains(d.IdOrdCli)).ToList()

                    ' D. Carichiamo i Contratti con le loro sotto-tabelle (Include mirati e leggeri)
                    OrdContext.AllordCliContratto _
                        .Include(Function(c) c.MaItems) _
                        .Include(Function(c) c.AlltipoRigaServizio) _
                        .Where(Function(c) ordIds.Contains(c.IdOrdCli)).ToList()

                    ' E. Carichiamo le Attività
                    OrdContext.AllordCliAttivita _
                        .Include(Function(a) a.Allattivita) _
                        .Where(Function(a) ordIds.Contains(a.IdOrdCli)).ToList()
                End If

                ' -------------------------------------------------------------------------
                ' 2. Ritorno i dati come IQueryable per non rompere il codice a valle
                ' -------------------------------------------------------------------------
                ' tempResult ora è una lista "piena" di tutti i dati collegati.
                ' La trasformiamo in IQueryable in modo che i tuoi metodi successivi funzionino.
                iAllOrders = tempResult.AsQueryable()
            End If
...
#End Region
                    ...
                    If o.ALLordCliContratto IsNot Nothing Then
...
#Region "Controllo Esclusioni righe Contratto"
                            ...
                            If c.AlltipoRigaServizio Is Nothing Then
                                errori.AppendLine("Ordine " & cOrd.OrdNo & " Tipo Riga Servizio Sconosciuta:" & c.TipoRigaServizio)
                                Debug.Print(" - [ESCLUSA] Tipo Riga Servizio Sconosciuta")
                                debugging.AppendLine(" - [ESCLUSA] Tipo Riga Servizio Sconosciuta")
                                Continue For
                            End If
                            ...
                            If c.DataDecorrenza > dataFattA Then
                                ...
                                Continue For
                            End If
                            If c.MaItems Is Nothing Then
                                errori.AppendLine("Ordine " & cOrd.OrdNo & " Servizio Assente:" & c.Servizio)
                                Debug.Print(" - [ESCLUSA] Servizio Sconosciuto")
                                debugging.AppendLine(" - [ESCLUSA] Servizio Sconosciuto")
                                Continue For
                            End If
...
#Region "STEP 3 Attività"
                            If c.AllordCliAttivita IsNot Nothing Then
                                For Each att In c.AllordCliAttivita
                                    ...
                                    If tipoAttivita = "X" Then
                                        errori.AppendLine("Ordine " & cOrd.OrdNo & " Attività Assente:" & att.Attivita)
                                        Debug.Print(" - [ESCLUSA] Attività Sconosciuto")
                                        debugging.AppendLine(" - [ESCLUSA] Attività Sconosciuto")
                                        Continue For
                                    End If
...
#End Region
                            ...
                            If Not cOrdRow.PrecendementeCessato AndAlso ((cOrdRow.IsOk AndAlso Not cOrdRow.CanoneFuoriRangeDate) OrElse cOrdRow.DaRifatturare) AndAlso (cOrdRow.QtaCorrente > 0 OrElse cOrdRow.QtaDaRifatturare > 0) Then
                                ...
                                isNewRows = True
                                If Not bScrittoDescrizioni And o.ALLordCliDescrizioni IsNot Nothing Then
...
#End Region
                ...
                Debug.WriteLine($"Ordini principali: {sw.ElapsedMilliseconds} ms )")
                sw.Stop()
#Region "Bulk Insert"
